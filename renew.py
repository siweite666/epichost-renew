...
# -*- coding: utf-8 -*-
"""
GODLIKE Public Page Renewal
通过 godlike.cool 公开续期页面续期，不依赖面板登录和广告系统。
reCAPTCHA 通过音频识别破解，IP 被封时用 WARP 换 IP 重试。

来源: https://github.com/oyz8/Godlike (renew.py)
"""
...
def build_caption(status: str, godlike_id: str, username: str,
    ...
    lines += ["", "Godlike Host Public Page Renew"]
    return "\n".join(lines)
...
# 单个 ID 续期
# ============================================================
def renew_single_id(godlike_id: str, tg_token: str, tg_chat_id: str) -> bool:
    """对单个 Godlike ID 执行续期"""
        ...
        try:
            ...
            try:
                ...
                screenshot_path = screenshot(
                    page, f"godlike-{godlike_id}-blocked-{attempt}.png"
                )
                page.quit()
                page = None
                restart_warp()
            ...
            if not solved:
                ...
                log(failure_reason, "WARN")
                screenshot_path = screenshot(
                    page, f"godlike-{godlike_id}-unsolved-{attempt}.png"
                )
                page.quit()
                page = None
                restart_warp()
            ...
            if "successfully renewed" in result_html.lower():
                log("✅ 续订成功！")
                success = True
                final_status = "success"
                screenshot_path = screenshot(
                    page, f"godlike-{godlike_id}-success.png"
                )
                break
                ...
                screenshot_path = screenshot(
                    page, f"godlike-{godlike_id}-maxed.png"
                )
                break
                ...
                screenshot_path = screenshot(
                    page, f"godlike-{godlike_id}-cooldown.png"
                )
                break
                ...
                screenshot_path = screenshot(
                    page, f"godlike-{godlike_id}-unknown-{attempt}.png"
                )
                page.quit()
                page = None
                restart_warp()
            ...
            if page:
                screenshot_path = screenshot(
                    page, f"godlike-{godlike_id}-error-{attempt}.png"
                )
            ...
            if page:
                if not screenshot_path:
                    screenshot_path = screenshot(
                        page, f"godlike-{godlike_id}-final-{attempt}.png"
                    )
                try:
                    page.quit()
                except Exception:
...
# ============================================================
def main():
    raw_ids = os.getenv("GODLIKE_ID", "").strip()
    ...
    godlike_ids = [i.strip() for i in raw_ids.replace(",", "\n").splitlines() if i.strip()]

    if not godlike_ids:
        log("GODLIKE_ID 解析为空，退出", "ERROR")
        sys.exit(1)

    log(f"共 {len(godlike_ids)} 个账号: {godlike_ids}")
    ...
    total_success = 0
    try:
        for idx, gid in enumerate(godlike_ids, 1):
            log(f"\n{'#'*60}")
            log(f"账号 {idx}/{len(godlike_ids)}: {gid}")
            log(f"{'#'*60}")
    ...
    log(f"\n全部完成：成功 {total_success}/{len(godlike_ids)}")
    if total_success < len(godlike_ids):
        sys.exit(1)
